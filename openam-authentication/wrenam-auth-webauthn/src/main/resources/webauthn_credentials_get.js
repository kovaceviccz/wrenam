(function () {
    ...
    const REASON_CREDENTIAL_NOT_FOUND = "CREDENTIAL_NOT_FOUND";
    ...
    const recoveryButton = document.getElementById("button_3");
    ...
    // success carries credential.toJSON(), while only terminal client failures are submitted as status:error.
    ...
    const toFailure = function (error) {
        ...
        case "InvalidStateError":
            return clientFailure(REASON_CREDENTIAL_NOT_FOUND);
        case "TypeError":
            return clientFailure(REASON_VERIFICATION_FAILED, CODE_INVALID_REQUEST);
        default:
            return clientFailure(REASON_VERIFICATION_FAILED, CODE_INVALID_REQUEST);
    ...
    let requestOptions;
    try {
        requestOptions = PublicKeyCredential.parseRequestOptionsFromJSON(optionsJson);
    } catch (error) {
    ...
    function startCredentialRequest() {
        if (completed) {
            return;
        }
        ...
        navigator.credentials.get(credentialRequest)
