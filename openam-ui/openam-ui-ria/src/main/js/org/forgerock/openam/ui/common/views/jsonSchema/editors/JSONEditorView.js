...
define([
    ...
    function convertHelpBlocksToPopOvers (element) {
        ...
        UIUtils.compileTemplate(template).then((html) => {
            $(element).find(".help-block:not(.errormsg)")
                .addClass("hidden-lg hidden-md hidden-sm").each((index, value) => {
    ...
    function applyJSONEditorToElement (element, schema, values, hideInheritance = false) {
        const GRID_COLUMN_WIDTH_1 = 6;
        const GRID_COLUMN_WIDTH_2 = 4;

        JSONEditor.plugins.selectize.enable = true;
        JSONEditor.plugins.selectize.plugins = (schema) => [{
            name: "enum_plugin",
            options: {
                schema
            }
